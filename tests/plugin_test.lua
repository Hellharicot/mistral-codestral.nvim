...
local function run_tests()
	...
	if auth_ok then
		...
		if api_key then
			...
			-- Skip API key validation in CI tests (requires valid API key)
	...
	-- Test 5: Check blink.cmp is available (optional in CI)
	local blink_cmp_ok = pcall(require, "blink.cmp")
	if not blink_cmp_ok then
		print("ℹ  Blink.cmp not installed (optional dependency)")
	else
	...
	-- Skip in CI environment as it requires a valid API key
	if ok and config and config.enabled and config.api_key ~= "test_key_for_ci" then
		...
		print("\n  Skipping actual completion request (requires valid API key)")
	end
