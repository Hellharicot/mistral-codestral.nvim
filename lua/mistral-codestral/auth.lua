...
function M.save_to_keyring(api_key)
	...
	if command_exists("security") then
		if vim.system then
			...
			local result = vim
				.system({ "security", "add-generic-password", "-s", service, "-a", username, "-w", api_key, "-U" }, { text = true })
				:wait()
			...
			if file then
				...
				local cmd = string.format(
					"security add-generic-password -s '%s' -a '%s' -w \"$(cat '%s')\" -U 2>/dev/null",
					service:gsub("'", "'\\''"),
					username:gsub("'", "'\\''"),
					temp_file:gsub("'", "'\\''")
				)
	...
	if command_exists("secret-tool") then
		if vim.system then
			...
			if file then
				...
				file:close()
				local cmd = string.format(
					"secret-tool store --label='Mistral Codestral API Key' service '%s' username '%s' < '%s' 2>/dev/null",
					service:gsub("'", "'\\''"),
					username:gsub("'", "'\\''"),
					temp_file:gsub("'", "'\\''")
				)
